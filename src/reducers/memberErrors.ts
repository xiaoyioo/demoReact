...
export const memberErrorsReducer = (state = createEmptyMemberErrors(), action: any) => {
  switch (action.type) {
...
const handleFetchMembersCompleted = (state: MemberErrors, payload: any) => {
...
const handleUpdateMemberField = (state: MemberErrors, payload: MemberFieldChangePayload): MemberErrors => {
  return {
    ...state,
    [(payload.fieldValidationResult.key as any)]: payload.fieldValidationResult,
  };
};

const handleSaveMember = (state: MemberErrors, payload: any): MemberErrors => {
  const newMemberErrors = { ...state };
  return payload.fieldErrors.reduce((memberErrors: any, fieldValidationResult: any) => {
